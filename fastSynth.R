fastSynth <-
  ...
  )
  {
    ...
    if(is.null(custom.v) & nrow(X0) != 1)
    {
      ...
      #                       bound.ipop = Bound.ipop
      # )
      rgV.optim.1 <- optimFunc(variablesv = SV1, X0scaled = as.matrix(X0.scaled), X1scaled = as.matrix(X1.scaled), as.matrix(Z0), as.matrix(Z1))
      cat('Okay')
      # get minimum
      ...
      if(inherits(Beta,"try-error"))
      {
        ...
        #                       sigf.ipop = Sigf.ipop,
        #                       bound.ipop = Bound.ipop)
        rgV.optim.2 <- optimFunc(variablesv = SV2, X0scaled = as.matrix(X0.scaled), X1scaled = as.matrix(X1.scaled), as.matrix(Z0), as.matrix(Z1))
        cat('Okay2')
        # get minimum
