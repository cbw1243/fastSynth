fastSynthV1 <-
  function(           data.prep.obj = NULL,
                      ...
                      Z1 = NULL,
                      custom.v = NULL,
                      optimxmethod = c("NLOPT_LN_PRAXIS", "NLOPT_LN_NELDERMEAD", 'NLOPT_LN_SBPLX'),
                      genoud = FALSE,
                      quadopt = "ipop",
                      Margin.ipop = 0.0005,
                      Sigf.ipop = 5,
                      ...
                      citation = FALSE,
                      ...){
if(isTRUE(citation)){cat(paste0('Fast computation of the synthetic contol method.',
               '\n Authors: Bowen Chen (PhD Candidate, Kansas State University);',
               '\n          Huayan Geng (PhD Candidate, SUNY Binghamton University). \n Please cite: XXX'))}
# profvis::profvis({ # For profiling.
    ...
    if(is.null(custom.v) & nrow(X0) != 1)
    {
      ...
      rgV.optim.1 <- nloptr( x0=SV1,
                             eval_f=fn.V,
                             opts=list("algorithm" = optimxmethod,
                                       "xtol_rel"=1.0e-8),
                            X0.scaled = X0.scaled,
                            X1.scaled = X1.scaled,
                            Z0 = Z0,
      ...
      if(inherits(Beta,"try-error"))
      {
        ...
        if(rgV.optim.1$objective < rgV.optim.2$objective) ###### Change from the above codes to this line.
        {
          rgV.optim <- rgV.optim.1
        } else {
      ...
      solution.v   <- abs(rgV.optim$solution)/sum(abs(rgV.optim$solution)) ##### Change from the above codes to this line.
